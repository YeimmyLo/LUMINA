...
const APPS_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbxzpG9wFgJizO8Nuya1awB2B-JLKEjEzXy2X8jXhE-TthsQlRpCwF4n1dTCpZI5c7xj/exec";

const EMAIL_PATTERN = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const FORM_TYPES = new Set<FormType>(["lead_hero", "contact", "newsletter_products"]);
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { ok: false, message: "No se pudo conectar con Google Sheets. Inténtalo más tarde." },
        { status: 502 }
      );
    }

    return NextResponse.json({ ok: true, message: "Información enviada correctamente." });
  } catch {
    return NextResponse.json(
      { ok: false, message: "No se pudo enviar la información. Revisa tu conexión e inténtalo de nuevo." },
      { status: 502 }
    );
  }
}
...
function validatePayload(body: unknown): ValidationResult {
  if (!isRecord(body)) {
    return { ok: false, message: "La solicitud no tiene un formato válido.", status: 400 };
  }

  if (typeof body.company === "string" && body.company.trim()) {
    return { ok: false, message: "No se pudo procesar la solicitud.", status: 400 };
  }
  ...
  if (!source || !page) {
    return { ok: false, message: "Faltan metadatos del formulario.", status: 400 };
  }

  if (!email || !EMAIL_PATTERN.test(email)) {
    return { ok: false, message: "Ingresa un correo electrónico válido.", status: 400 };
  }
  ...
  if (formType === "contact" && (!subject || !message)) {
    return { ok: false, message: "Completa el asunto y el mensaje.", status: 400 };
  }
